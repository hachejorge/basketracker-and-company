...
CREATE TABLE sisinf_db.EQUIPO (
    ...
    ubicacion TEXT,
    competicion TEXT REFERENCES COMPETICION(nombre) ON DELETE CASCADE
);

-- Crear la tabla JUGADOR
CREATE TABLE sisinf_db.JUGADOR (
    nombre_usuario TEXT PRIMARY KEY REFERENCES USUARIO(nombre_usuario) ON DELETE CASCADE,
    nombre_jugador TEXT,
    equipo INTEGER REFERENCES EQUIPO(id_equipo) ON DELETE CASCADE
);

-- Crear la tabla PARTIDO
CREATE TABLE sisinf_db.PARTIDO (
    id_partido INTEGER PRIMARY KEY,
    equipo_local INTEGER REFERENCES EQUIPO(id_equipo) ON DELETE CASCADE,
    equipo_visitante INTEGER REFERENCES EQUIPO(id_equipo) ON DELETE CASCADE,
    jornada INTEGER,
...
CREATE TABLE sisinf_db.PTS_JUG_PAR (
    ...
    nombre_usuario TEXT REFERENCES JUGADOR(nombre_usuario) ON DELETE CASCADE,
...
CREATE TABLE sisinf_db.COMENTARIO (
    nombre_usuario TEXT REFERENCES USUARIO(nombre_usuario) ON DELETE CASCADE,
    id_partido INTEGER REFERENCES PARTIDO(id_partido) ON DELETE CASCADE,
...
CREATE TABLE sisinf_db.JUGADOR_FAV (
    nombre_usuario TEXT REFERENCES USUARIO(nombre_usuario) ON DELETE CASCADE,
    jugador TEXT REFERENCES JUGADOR(nombre_usuario) ON DELETE CASCADE,
